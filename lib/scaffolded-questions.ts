...
export const LEVEL1_QUESTIONS: Record<Module, QuizQuestion[]> = {
  bartending: [
    ...
    { question: "An Espresso Martini is traditionally made with vodka, coffee liqueur, and freshly pulled espresso.", answer: true, explanation: "The Espresso Martini (created by Dick Bradsell in London) combines vodka, coffee liqueur, and hot espresso, shaken hard to produce the signature crema on top.", tags: ["cocktails", "espresso-martini", "classics"] },
...
export const LEVEL2_DESCRIPTORS: Record<Module, DescriptorQuestion[]> = {
  bartending: [
    ...
    {
      prompt: "A guest orders an Espresso Martini. Which two steps are non-negotiable for the signature frothy crema on top?",
      descriptors: ["Use freshly pulled espresso (still hot)", "Shake vigorously with ice for 15–20 seconds", "Stir gently to avoid over-dilution", "Add cream to the shaker", "Use cold brew concentrate instead of espresso"],
      correctIndices: [0, 1],
      explanation: "Hot espresso + a hard shake creates the emulsified foam layer that defines a great Espresso Martini. Cold ingredients or a gentle shake will produce little to no crema.",
...
export const LEVEL3_DESCRIPTORS: Record<Module, DescriptorQuestion[]> = {
  bartending: [
    ...
    },
    {
      ...
      descriptors: ["Use a freshly pulled espresso shot (still hot) for maximum crema", "Shake extremely hard with ice for 15–20 seconds", "Fine strain into a chilled coupe or martini glass to preserve the foam", "Stir gently to avoid breaking the crema", "Add milk to soften the coffee flavour"],
      correctIndices: [0, 1, 2],
      explanation: "Fresh hot espresso, a vigorous shake, and fine straining into a chilled glass are the three non-negotiables. Hot espresso + hard shake = emulsified foam. Cold espresso or gentle mixing gives you none of that.",
      tags: ["cocktails", "espresso-martini", "technique"],
    },
    {
