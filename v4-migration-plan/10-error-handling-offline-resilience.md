...
## Implementation Notes (Day 6 — 2026-08-19, second pass) — runtime safety audit (hydration / boundary / route exceptions)

A narrower follow-up to the audit above, scoped to runtime *crashes* rather than network resilience: SSR/CSR hydration mismatches, unguarded lookups against `lib/arena-scenarios.ts`, and exception safety in the Fal.ai route. Same constraint as before — defensive guards only, no layout or business-logic rewrites. Three real defects found and fixed.

**1. Hydration mismatch in `MatchPairsScreen.tsx` (the only one in `app/mobile`).** The deck was built via `useState<Card[]>(() => buildDeck())`, and `buildDeck()` ran a `Math.random()` Fisher-Yates shuffle. Every `app/mobile` route is SSR'd (these are `"use client"` components, but Next.js still server-renders them — `app/mobile/match-pairs/page.tsx` renders the screen directly with no Suspense boundary), so the server produced one card order and the client produced a different one — a mismatch across all 12 card labels on every load. Fixed by splitting `buildDeck(shuffle: boolean)`: the initial state uses the deterministic order (all 6 ingredients, then all 6 cocktails — pair `i` sits at index `i` and `i + 6`, two rows apart in the 3×4 grid, so the pre-hydration HTML never reveals a pairing), and the real shuffle moved into a mount effect. `reset()` ("Play again") still shuffles as before.

**Swept the rest of `app/mobile` for the same class and found nothing else.** Every `localStorage` read is already inside a `useEffect` or an event handler (`use-challenge-complete.ts`, `ChallengesScreen.tsx`, `MatchPairsScreen.tsx`, and `BadgesGalleryScreen.tsx`'s `null`-skeleton streak read, which was already explicitly written SSR-safe). `AiProfilePhotoScreen.tsx`'s `new window.Image()` / `document.createElement("canvas")` live in `downscaleImage()`, only reachable from a file-input change handler. Also grepped specifically for time-based render (`getHours`, `toLocale*`, `Intl.*`, `new Date()`) — no time-of-day greeting or locale-formatted timestamp anywhere in the mobile tree, so that common mismatch source doesn't apply here.

**2. Unguarded `ARENA_SEED_SCENARIOS` lookup in `ArenaPage.tsx:93` (desktop).** `const seed = ARENA_SEED_SCENARIOS[selectedId]!` — a non-null assertion on a `Record<number, …>` index. A module id with no seed content reached `formatArenaScenario(seed)` as `undefined` and threw `TypeError: Cannot read properties of undefined`; the surrounding `catch` turned that into a raw TypeError string in the user-facing error banner. Replaced with a real `if (!seed)` guard and a clear message. The `finally { setSubmitting(false) }` already covers the early return, so no hung spinner. Every other call site was already correct: mobile's `LearnHubScreen`/`ScenarioTrainingScreen` guard with `if (!seed) return` / `.filter()`, and `ArenaPage`'s own writing-phase render (line ~393) already wraps in `{seed && (…)}`. This is now consistent with the file header's own instruction to treat the lookup as one that can miss.

**3. Malformed request body in `app/api/profile-photo/generate/route.ts` returned 500, not 400.** `await req.json()` sat bare inside the outer try, so an empty or non-JSON body threw into the catch-all and came back as a 500 "Generation failed. Please try again." — a client error reported as a server fault, and one more way the "not working" report gave no usable signal. Now `.catch(() => null)` with an explicit 400 "Invalid request body." The rest of the route's exception surface was already correct and needed no change: missing/invalid auth → 401, bad base64 or oversized selfie → 400 via `parseSelfieDataUrl`'s regex + length cap, an unknown `styleId` → 400, and network timeouts / Fal rejections → the `ApiError`/`ValidationError`-branching 500 added in the previous pass. Every path returns JSON; nothing can escape as an HTML error page.

**Deliberately not changed**: the audit brief suggested a `{ success: false, error: string }` response shape. This route (and every other API route in the codebase) returns bare `{ error: string }`, and `AiProfilePhotoScreen.tsx` reads `data.error`/`data.detail` directly — switching the shape here alone would break the client and diverge from the project-wide convention for no gain.

Verified clean: `npx tsc --noEmit`, `npm run lint`, full `npx next build` — all pass with zero errors/warnings.
