...
## Implementation Notes (Day 6 — 2026-08-19) — remaining 4 games built

User asked directly: "Can you build the other 4 games?" All 4 built this pass, closing out the last inert rows on `ChallengesScreen`.

- **Real content ported verbatim from desktop**, not invented: `SequenceSortScreen.tsx` (challengeIndex 0, from `SequenceSortGame.tsx`'s Guinness/Margarita/Pinot Grigio pour-order question), `FillBlankScreen.tsx` (challengeIndex 1, from `FillBlankGame.tsx`'s Classic Daiquiri recipe reconstruction), `SpotErrorScreen.tsx` (challengeIndex 3, from `SpotErrorGame.tsx`'s bottled-vs-fresh-lime-juice error card), `MultipleChoiceScreen.tsx` (challengeIndex 4, from `MultipleChoiceGame.tsx`'s lipstick-mark guest scenario). Same questions, same correct answers, same explanations as desktop — scoring stays consistent across surfaces.
- **Desktop's flow is a sequential 5-question wizard** (`ChallengesPage.tsx` steps through all 5 in order, one summary at the end); mobile's is 5 independently-playable standalone routes (the model `MatchPairsScreen.tsx` already established in file `05`'s first pass). Kept mobile consistent with its own established pattern rather than porting the wizard shape — each game is its own route, "complete" = answered correctly (with unlimited retry via a Try Again button, matching desktop's own retry-until-correct behavior), not step N of 5.
- **Two new shared files** factored out rather than duplicating boilerplate 4 times: `app/mobile/_lib/use-challenge-complete.ts` (`useMarkChallengeComplete(challengeIndex, isComplete)` — the same localStorage + `POST /api/training/challenges/save` sync `MatchPairsScreen.tsx` already does inline, factored out so the new games share it; `MatchPairsScreen.tsx` itself left untouched, not worth the regression risk of refactoring a already-shipped, working file for this) and `app/mobile/_components/challenges/MobileChallengeChrome.tsx` (mobile-dark `FeedbackBanner`/`TryAgainButton`/`CompletionCard` — the dark-theme equivalent of desktop's `ChallengeCard.tsx` shared shell, restyled to `--*-mobile` tokens rather than ported 1:1 since desktop's version is light-theme only). `CompletionCard` includes the `scrollIntoView` fix from the same-day Match Pairs bug fix (completion state was rendering below the fold with no scroll cue) baked in from the start.
- **New routes**: `/mobile/recipe-order`, `/mobile/memory-test`, `/mobile/menu-audit`, `/mobile/speed-round`. `ChallengesScreen.tsx`'s 4 previously-disabled rows now all route to their real game.
...
- **Still open:** live verification that all 4 new games' completions correctly upsert `user_challenges` at their respective indices and increment `ProgressScreen`'s count — same category as Match Pairs' own still-open step 6.
