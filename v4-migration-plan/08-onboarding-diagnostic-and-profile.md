...
## Implementation Notes (Day 6 — 2026-08-19) — real camera/selfie capture

User asked directly for this scoped item after the live-QA round: "Real camera/selfie capture for AI Photo — copy fixed, feature doesn't exist." True at the time: generation was pure text-to-image (a style prompt only), so a camera control would have had nothing to feed. Scoped and built this pass rather than left open.

- **Client**: hidden `<input type="file" accept="image/*" capture="user">` (triggers the native camera on a phone browser; falls back to the file picker on desktop) wired to a camera-icon button on the preview circle. Selected photo is downscaled client-side (longest edge capped at 768px, JPEG 0.85) before ever leaving the device — a raw phone photo can be several MB, the model only needs a modest reference image. Shows a small thumbnail + "Remove photo" once captured; copy above the preview changes to reflect whether a photo is in play.
- **Server (`/api/profile-photo/generate`)**: now accepts an optional `selfieImage` (validated as a `data:image/(png|jpeg|webp);base64,…` string, size-capped as a backstop against a modified client). When present, generation switches from `fal-ai/flux/schnell` (text-to-image) to `fal-ai/flux/dev/image-to-image`, passing the selfie as `image_url` (a `Blob`, auto-uploaded to Fal's storage by the client SDK) and the existing style prompt as the transformation prompt, `strength: 0.75`. Without a selfie, behavior is byte-for-byte unchanged from before (still schnell, still text-to-image) — this is additive, not a replacement path.
- **Deliberately not built**: any Supabase Storage bucket or DB column for the raw selfie. It's never persisted by this app — only sent once to the generate route for that single Fal call. `save/route.ts`'s existing `isAllowedFalUrl()` check (unchanged) already only accepts `*.fal.media` URLs, so a raw selfie could never become `profiles.profile_photo_url` even if a future bug tried to save one directly — the AI-generated output is the only thing that can ever be saved, same invariant as before this feature.
- **Cost note for the user to be aware of**: `flux/dev/image-to-image` is priced higher than `flux/schnell` per call. The existing 5/min per-user and per-IP rate limits are unchanged and still apply to both paths.
- Verified clean: `npx tsc --noEmit`, `npx eslint app/mobile app/api/profile-photo --max-warnings=0`, and a full `npx next build` — all pass with zero errors/warnings.
- **Still open**: a live signed-in browser pass — capture a photo, confirm the image-to-image call returns a recognizable-but-stylized result, confirm the non-photo path still works unchanged, confirm rate limiting still trips. Same category as this file's other still-open verification items.
